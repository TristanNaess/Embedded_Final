...
{
    s->x_div = (s->x_max - s->x_min) >> 3;
    s->y_div = (s->y_max - s->y_min) >> 3;
}

// get value, [-4,4] of axis
uint8_t get_region(Stepper * s, uint8_t axis)
{
    if (axis) // y
    {
        if (s->y > s->y_zero + 1) // positive
        {
        ...
        else if (s->y < s->y_zero - 1) // negative
        {
            if (s->y > s->y_zero - (1 * s->y_div))
                return 0;
            else if (s->y > s->y_zero - (2 * s->y_div))
    ...
    else // x
    {
        ...
        else if (s->x < s->x_zero - 1) // negative
        {
            if (s->x > s->x_zero - (1 * s->x_div))
                return -1;
            else if (s->x > s->x_zero - (2 * s->x_div))
